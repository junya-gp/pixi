...
(async () =>
{
	...
	const brush = new Graphics().circle(0, 0, 10).fill({ color: 0xffffff });

	// Create a line that will interpolate the drawn points
	const line = new Graphics();

	// Load the textures
	await Assets.load(['scratch.jpg', 'https://pixijs.com/assets/bg_rotate.jpg']);
  ...
  const { width, height } = app.screen;
	...
	const background = Object.assign(Sprite.from('scratch.jpg'), stageSize);
	//const background = new Graphics().rect(0, 0, width, height).fill({ color: 0xffffff });
	...
	function pointerMove({ global: { x, y } })
	{
			if (dragging)
			{
					...
					if (lastDrawnPoint)
					{
							line.clear().moveTo(lastDrawnPoint.x, lastDrawnPoint.y).lineTo(x, y).stroke({ width: 20, color: 0xffffff });
							app.renderer.render({
									container: line,
									target: renderTexture,
									clear: false,
